...
HeaderPilot collects nothing and uploads nothing. All rules are stored locally via `chrome.storage.local`. See [`privacy-policy.html`](https://kaiby.github.io/headerpilot/privacy-policy.html) for the full policy.
...
HeaderPilot 不收集、不上传任何数据，所有规则均通过 `chrome.storage.local` 保存在本地。完整政策见 [`privacy-policy.html`](https://kaiby.github.io/headerpilot/privacy-policy.html)。
